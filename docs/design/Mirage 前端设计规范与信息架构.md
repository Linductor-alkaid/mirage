...
> 2026-09-18 修订：内置主题扩为 6 套，新增「任务控制台」（mirage-console，Mission
> Console 视觉世界）为默认主题；主题机制与门槛不变。实现事实源：
> 仓库根 `DESIGN.md` 与 `ui/app/src/theme/`。
...
- 2026-09-18：`M1.5-07` 重写落地「任务控制台」视觉世界（Apollo-era Mission
  Console 语系：壁挂屏状态动词 + 蓝线任务剖面 + 告警灯阵、暖琥珀唯一行动色、
  炭蓝控制台底 + 奶油面板、Saira Condensed/Chivo Mono 本地字体）。组件栈定选
  见 [DEC-014](../decisions/DEC-014-ui-component-stack.md)；设计系统实现事实源
