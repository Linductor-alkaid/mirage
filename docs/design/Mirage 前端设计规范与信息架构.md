...
**专业、安静、证据先行。** 低饱和中性底色 + 单一品牌蓝作为唯一强调来源；状态色
（成功/警告/危险/信息）只用于状态语义，不做装饰。界面密度中高（桌面生产力工具），
圆角中等，阴影极轻，以 1px 边界划分区域。
...
| 外观 Appearance | 主题三态、强调色（品牌蓝锁定）、密度、字号缩放、等宽字体 | 本文 §2 |
...
- **M1.5（浏览器形态）**：`M1.5-07` 统一壳与信息架构重构（ActivityBar/StatusBar/
  路由表/会话侧栏骨架 + 既有视图迁入）；`M1.5-08` 设计 token 与组件库落地
  （本文 §2 全量 token 进 `ui/`，StatusBadge/SnapshotFrame(占位)/ActivityCard 等
  基础组件）；工作项定义见 M1.5 计划修订记录。
