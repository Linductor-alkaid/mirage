...
LinuxDesktopEnvironment::read_text_file(const std::filesystem::path& path) {
    ...
    if (stream.bad()) {
        return {false, {}, provider_error("io_error", "read failed: " + path.string())};
    }
...
LinuxDesktopEnvironment::execute(const std::string& command,
    ...
    if (limits.timeout <= std::chrono::milliseconds::zero() ||
        ...
        return {false, false, false, false, -1, {}, {},
                provider_error("invalid_argument",
                               "timeout and output budget must be positive")};
    }
    ...
    if (pid == 0) {
        // Child: own process group so the budget kill reaches the whole
        // command tree; dup2 clears O_CLOEXEC on the duplicated ends.
        setpgid(0, 0);
    ...
    if (open_streams > 0) {
        // Timeout or poll failure: stop reading and tear the command down.
        if (open_streams & 1) {
            close(stdout_pipe[0]);
        }
        if (open_streams & 2) {
    ...
    // Whole-group teardown: a budget-covered command must leave nothing
    // behind, so the process group is SIGKILLed unconditionally before
    ...
    if (timed_out) {
        return {false, true, output_truncated, false, -1, std::move(standard_output),
                std::move(standard_error),
                provider_error("deadline_exceeded", "command exceeded its time budget")};
    }
    if (!reaped) {
        return {false, false, output_truncated, false, -1, std::move(standard_output),
                std::move(standard_error),
                provider_error("io_error", "child could not be reaped")};
    }
    if (WIFSIGNALED(status)) {
        return {false, false, output_truncated, false, 128 + WTERMSIG(status),
